...
from src.domain.validation.parameter_specs import get_parameter_spec
from src.domain.validation.rules import EnumRule, RangeRule
...
async def list_calculators(
    limit: int = Query(200, ge=1, le=250, description="Maximum results"),
...
def _input_schema(calculator: Any) -> list[dict[str, Any]]:
    """Build UI-safe input metadata from the calculator signature."""
    signature = inspect.signature(calculator.calculate)
    hints = get_type_hints(calculator.calculate)
    ...
    for name, parameter in signature.parameters.items():
        ...
        if origin is Literal:
            options = args
            value_type = type(options[0]) if options else str
        else:
        ...
        if spec is not None:
            ...
            for rule in spec.rules:
                ...
                elif isinstance(rule, EnumRule) and not options:
                    options = list(rule.allowed_values)
        ...
        fields.append(
            {
                ...
                "required": parameter.default is inspect.Parameter.empty and not nullable,
                "default": default,
                "unit": unit,
                "minimum": minimum,
                "maximum": maximum,
...
@app.get("/api/v1/calculators/{tool_id}/schema", tags=["Discovery"])
async def get_calculator_schema(tool_id: str, registry: Any = Depends(get_registry_from_app)) -> dict[str, Any]:
    """Return typed input metadata for dynamic clinical forms."""
    calculator = registry.get_calculator(tool_id)
    if calculator is None:
        raise HTTPException(status_code=404, detail=f"Calculator '{tool_id}' not found")

    return {"tool_id": tool_id, "inputs": _input_schema(calculator)}
