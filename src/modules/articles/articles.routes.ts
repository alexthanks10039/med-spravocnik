...
articlesRouter.get('/:id', async (req, res, next) => {
  try {
    const x = await prisma.article.findUnique({ where: { id: req.params.id } });
    if (!x || !x.isPublished) throw new AppError('Article not found', 404);
    res.json(x);
  } catch (e) {
    next(e);
  }
...
articlesRouter.post('/', authMiddleware, adminMiddleware, async (req, res, next) => {
  try {
    ...
    res.status(201).json(await prisma.article.create({ data: d }));
  } catch (e) {
    next(e);
  }
});
