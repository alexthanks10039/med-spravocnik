            ...
            actions: [
              IconButton(
                ...
                onPressed: () {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Фрагмент скопирован')));
                },
                icon: const Icon(Icons.content_copy_rounded),
              ),
              IconButton(
                tooltip: favorite ? 'Убрать из избранного' : 'В избранное',
                onPressed: () => ref.read(favoriteIdsProvider.notifier).toggle(data.id),
                icon: Icon(favorite ? Icons.bookmark_rounded : Icons.bookmark_border_rounded),
              ),
            ],
          ),
                        ...
                        children: [
                          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            ...
                            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              Semantics(header: true, child: Text(data.title, style: Theme.of(context).textTheme.headlineMedium)),
                              const SizedBox(height: 6),
                              Text(data.subtitle, style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant)),
                              if (data.badge != null) ...[const SizedBox(height: 10), Chip(label: Text(data.badge!))],
                            ])),
                          ]),
...
class _SectionCard extends ConsumerWidget {
  const _SectionCard({required this.documentId, required this.title, required this.text, required this.initiallyExpanded, required this.jumpTo});
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final saved = ref.watch(bookmarkProvider.select((state) => state[documentId]?.contains(title) ?? false));
    return Padding(
      ...
      child: Card(
        child: ExpansionTile(
          ...
          trailing: Semantics(
            button: true,
            label: saved ? 'Удалить закладку для раздела $title' : 'Добавить закладку для раздела $title',
            child: IconButton(
              tooltip: saved ? 'Удалить закладку' : 'Добавить закладку',
              onPressed: () => ref.read(bookmarkProvider.notifier).toggle(documentId, title),
      ...
      child: Card(
        ...
        child: Padding(
          ...
          child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('Красные флаги', style: Theme.of(context).textTheme.titleMedium),
