...
class SavedScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return ScreenFrame(title: 'Сохранённое', child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        Expanded(child: _Shortcut(icon: Icons.history_rounded, title: 'История', onTap: () => context.go('/history'))),
        const SizedBox(width: 12),
        Expanded(child: _Shortcut(icon: Icons.bookmarks_outlined, title: 'Закладки', onTap: () => context.go('/bookmarks'))),
      ]),
      const SizedBox(height: 12),
      Row(children: [Expanded(child: _Shortcut(icon: Icons.note_alt_outlined, title: 'Заметки', onTap: () => context.go('/notes')))]),
...
class _NotesScreenState extends State<NotesScreen> {
  ...
  @override Widget build(BuildContext context) => ScreenFrame(title: 'Заметки', child: Column(children: [
    Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(children: [TextField(controller: controller, maxLines: 4, decoration: const InputDecoration(hintText: 'Новая клиническая заметка...')), const SizedBox(height: 12), Align(alignment: Alignment.centerRight, child: FilledButton.icon(onPressed: () { if (controller.text.trim().isEmpty) return; setState(() { notes.insert(0, controller.text.trim()); controller.clear(); }); }, icon: const Icon(Icons.add_rounded), label: const Text('Сохранить')))]))),
    const SizedBox(height: 16), if (notes.isEmpty) const StatePanel.empty(title: 'Заметок пока нет', message: 'Добавьте личную заметку к материалу или создайте её здесь.'),
    ...notes.map((note) => Padding(padding: const EdgeInsets.only(bottom: 10), child: Card(child: ListTile(contentPadding: const EdgeInsets.all(16), leading: const Icon(Icons.sticky_note_2_outlined), title: Text(note), subtitle: const Padding(padding: EdgeInsets.only(top: 8), child: Text('Сохранено на устройстве')), trailing: IconButton(icon: const Icon(Icons.more_horiz), onPressed: () {}))))),
  ]));
}
...
class _Shortcut extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Card(child: InkWell(borderRadius: BorderRadius.circular(20), onTap: onTap, child: Padding(padding: const EdgeInsets.all(18), child: Column(children: [Icon(icon, color: Theme.of(context).colorScheme.primary), const SizedBox(height: 8), Text(title, style: Theme.of(context).textTheme.titleMedium)]))));
}
